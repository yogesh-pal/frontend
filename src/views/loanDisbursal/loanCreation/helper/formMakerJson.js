...
import {
  ...
  calculateNetDisbursement, formulaOfNetDisbursement, handleReset, updateLabel,
  calculateNetDisbursementOnline, handleLoanAccount, handleLoanAccountDelete, upiVerify, upiOptionhandler, calculateCashHandlingCharge, calculateNetCashDisbursement, CASH_HANDLING
} from './helperFunction';
import {
  AppIdCreationAPi, CollateralDataSAveAPi, ConsolidateDataSAveAPi, GoldInfoDataSAveAPi,
  schemeDataSAveAPi, FeeDataSAveAPi, InterAccountDataSAveAPi, BalanceTransferDataSAveAPi,
  AdditionalGoldDataSAveAPi, finalDataSAveAPi, isAllowed
} from './apiFunction';
...
const nextHandler = (
  ...
  const { loanMaker } = state;
  switch (currentStep) {
    case 0:
      return AppIdCreationAPi({
        applied_loan_amount: step['Applied Loan Amount'].applied_laon_amount,
        customer_id: loanMaker.customerId,
        colender: step['Applied Loan Amount'].applied_for,
        partner_cuid: step['Applied Loan Amount'].partner_cuid,
        insurance: losConfigData[step['Applied Loan Amount'].applied_for]?.insurance,
        losConfigData
      }, setAlertShow, reset, losConfigData[step['Applied Loan Amount'].applied_for]?.required_fields?.LOS?.includes('employee_dedupe' ?? false));
    case 1:
      return CollateralDataSAveAPi(step, setAlertShow, reset);
    case 2:
      return ConsolidateDataSAveAPi(step, setAlertShow, reset);
    case 3:
      return GoldInfoDataSAveAPi(step, setAlertShow, reset, allStepsData, setIsLoading);
    case 4:
      return schemeDataSAveAPi(step, setAlertShow, reset);
    case 5:
      return FeeDataSAveAPi(step, setAlertShow, reset, losConfigData[step['Applied Loan Amount'].applied_for]?.insurance);
    case 6:
      return InterAccountDataSAveAPi(step, setAlertShow, reset, losConfigData);
    case 7:
      return BalanceTransferDataSAveAPi(step, setAlertShow, reset, null, losConfigData);
    case 8:
      return AdditionalGoldDataSAveAPi(step, setAlertShow, reset);
    case 9:
      return finalDataSAveAPi(step, setAlertShow, reset, setIsLoading, navigate);
    default:
...
const handleDynamicValidationInter = (values, setError, setAlertShow) => {
  ...
  let requestAmount = values.requested_gold_loan_amount - values.total_fees;
  ...
  collateralData?.forEach((item) => {
    ...
    if (loanToCheck?.length && parseFloat(values[`amount_to_transfer${index}`].replace(/,/g, '')) > Math.ceil(loanToCheck[0].total_outstanding_for_foreclosure)) {
      ...
      response = true;
    } else if (requestAmount < values[`amount_to_transfer${index}`]) {
...
const fetchSchemeAndSaveInRedux = async (
  userDetails,
  saveSchemeData,
  values,
  callback,
  ...
  try {
    ...
    if (formData.end_use_loan === 'BNS') {
      ...
      //   setValue('udyam_certificate', formData.end_use_loan_details.proof);
      // } else {
      setValue('udyam_registration_number_verify', formData.end_use_loan_details.status);
      formData?.end_use_loan_details?.proof?.forEach((ele, ind) => {
        if (ind) {
          setValue(`udyam_certificate__other_${ind}`, ele);
        } else {
    ...
    const requestBody = {
      ...
      is_active: true,
      colender: values.applied_for
    };
    const { data } = await Service.post(`${process.env.REACT_APP_SCHEME_LIST}?page_size=${1000}&page=${1}`, requestBody);
    ...
    setAlertShow({ open: true, msg: 'Something went wrong while fetching scheme details.', alertType: 'error' });
  }
};

const netDisbursementOnback = (unregister, formDetails, setValue) => {
  const st = store.getState();
  const r = st.loanMaker;
  const val = r?.formData?.bundled_insurance ? 'YES' : 'NO';
  setValue('bundled_insurance', val);
};

export const formJsonDetails = (resendReducer, dispatch, setAlertShow, setFormDetails, appliedLaonAmount, losConfigData, ornamentsList, saveSchemeData, setIsLoading, cityCodeMapping, navigate) => {
  const appliedForOptions = [];
  const insurance = [];
  Object.keys(losConfigData).forEach((ele) => {
    if (ele !== '_default' && ele !== 'SHIVALIK') {
      appliedForOptions.push({ label: losConfigData[ele].display_name, value: ele });
    }
    insurance.push({ label: ele, value: [losConfigData[ele].insurance] });
    console.log('APPLIED FOR', appliedForOptions, insurance);
  });

  const sumInsuredSetter = (values, callback) => {
    try {
      const premiumData = losConfigData?.[values.applied_for]?.insurance;
      const sumInsuredOptions = premiumData?.map((ele) => (ele.sum_insured));
  ...
  // console.log('form Data is', formData, udyam_data);
    ...
    if (udyamData?.mode === 'Online') {
      udyamFields = [
        ...
        },
        {
          ...
          function: (value) => udyamSendOtp(value, setAlertShow, formData, dispatch, resendReducer),
      ...
      udyamFields = [
        ...
        },
        {
          ...
          // condition: {
          //   type: 'showOnValue',
          //   baseOn: 'udyam_manual_upload_postotp',
          //   baseValue: true
          // },
          maxUploadCount: 5,
  ...
  const goldInfoBackDynamicHandler = () => {
    if (!udyamData?.proof) {
      return {
        unregisterFields: [
  ...
  const deviationData = [
    ...
    },
    {
      ...
      validation: {
        isRequired: true,
        requiredMsg: 'Please enter Gold Pouch Number',
        pattern: REGEX.SHIVALIKGOLDPOUCHNUMBER,
        patternMsg: 'Gold pouch number is invalid',
      },
      condition: {
        type: 'showOnValue',
  ...
  const formConfiguration = {
    form: [
      {
        ...
        input: [
          {
            name: 'applied_for',
            label: updateLabel('Applied For'),
            type: 'select',
            identifier: IDENTIFIER.SELECT,
            multiSelect: false,
            option: appliedForOptions,
            defaultValue: '',
            validation: {
              isRequired: true,
              requiredMsg: 'Please select applied for'
            }
          },
          {
            name: 'partner_cuid',
            label: updateLabel('Partner CUID'),
            type: 'text',
            identifier: IDENTIFIER.INPUTTEXT,
            defaultValue: '',
            validation: {
              isRequired: true,
              requiredMsg: 'Please enter partner cuid',
              pattern: REGEX.NUMBER,
              patternMsg: 'Please enter numeric digits only',
              maxLength: 6,
              maxLenMsg: 'Maximum 6 digits allowed',
            },
            condition: {
              type: 'showOnValue',
              baseOn: 'applied_for',
          ...
          },
          {
            ...
            label: updateLabel('Applied Loan Amount'),
            type: 'text',
            identifier: IDENTIFIER.INPUTTEXT,
            isAmount: true,
            defaultValue: appliedLaonAmount ?? '',
            validation: {
              isRequired: true,
              requiredMsg: 'Please enter Applied Loan Amount',
              pattern: REGEX.AMOUNT,
              patternMsg: 'Please enter numeric digits only',
              min: 1,
              minMsg: 'Applied Loan Amount cannot be o.',
            },
            InputProps: {
              possition: 'start',
              text: '₹'
      ...
      },
      {
        ...
        input: [
          ...
          // {
          //   name: 'net_weight_before_purity',
          //   label: updateLabel('Net Weight before purity Conversion'),
          //   type: 'text',
          //   identifier: IDENTIFIER.INPUTTEXT,
          //   disabled: true,
          //   defaultValue: 0,
        ...
        input: [
          {
            ...
            label: updateLabel('Total Net Count of All items'),
            type: 'text',
            identifier: IDENTIFIER.INPUTTEXT2,
            disabled: true,
      ...
      },
      {
        ...
        input: [
          ...
          },
          {
            ...
            option: [
              { value: 'HOM', label: 'Home Renovation' },
              { value: 'MED', label: 'Medical Emergency' },
              { value: 'TRV', label: 'Travel' },
          ...
          },
          {
            ...
            option: [
              { value: 'PNL', label: 'Personal Needs' },
              { value: 'EDN', label: 'Education Purpose' },
              { value: 'MED', label: 'Medical Treatment' },
              { value: 'HOM', label: 'House Purchase or Maintenance' },
              { value: 'MRG', label: 'Marriage' },
              { value: 'DOM', label: 'Domestic Expenses' },
            ],
            validation: {
              isRequired: true,
              requiredMsg: 'Please select End use of Gold',
          ...
          },
          {
            ...
            option: [
              ...
              { value: 'Procurement of seeds', label: 'Procurement of seeds' },
              { value: 'Procurement of Fertilizers', label: 'Procurement of Fertilizers' },
              { value: 'Procurement of cattle feeds', label: 'Procurement of cattle feeds' },
              { value: 'Purchase of Agri Equipment/Machinery', label: 'Purchase of Agri Equipment/Machinery' },
              { value: 'For Agriculture Input/Farm Product', label: 'For Agriculture Input/Farm Product' }
            ],
            validation: {
              isRequired: true,
          ...
          },
          {
            ...
            option: [
              ...
              { value: 'Procurement of Raw Material/Goods', label: 'Procurement of Raw Material/Goods' },
          ...
          ...udyamFields,
          {
            ...
            // },
            condition: {
              type: 'showOnValueMultiple',
              baseOn: [{ name: 'end_use_of_gold', value: 'BNS' },
                { name: 'applied_for', value: appliedForOptions.filter((option) => option.value !== 'IOB - BUSINESS') },
              ],
            },
            isVerify: true,
            inputField: {
        ...
        input: [
          {
            ...
            custominitFunction: (values, callback, setValue) => fetchSchemeAndSaveInRedux(userDetails, saveSchemeData, values, callback, setAlertShow, setIsLoading, setValue),
      ...
      },
      {
        ...
        input: [
          ...
          },
          {
            ...
            functionMethod: FUNCTION_IDENTIFIER.ON_CHANGE,
            customFunction: (formValues, callback) => {
              handleLoanKawach(formValues, callback, insurance?.find((item) => item.label && item.label.includes(formValues.applied_for))?.value[0] || []);
            },
        ...
        onBack: {
          resetAdditionalForm: true,
          reset: {
            currentForm: ['balance_transfer_mode', 'sum_insured', 'premium_amount'],
          },
          resetFieldValidation: {
            amount: {
              isRequired: false
      ...
      },
      {
        ...
        variant: 'outlined',
        onBack: {
          dynamicHandler: netDisbursementOnback,
          disable: {
            value: false,
            disableFields: ['upiId'],
          },
    ...
    stepper: {
      steps: ['Applied Loan Amount', 'Collateral Details', 'Consolidated Collateral Details', 'Gold Information',
    ...
    nextFunction: (step, currentStep, reset, allStepsData) => nextHandler(step, currentStep, setAlertShow, reset, allStepsData, setIsLoading, navigate, losConfigData),
