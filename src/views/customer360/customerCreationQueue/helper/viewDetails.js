...
export const formJsonDetails = (userDetails, handleFilePreview) => {
    ...
    tempForm.form.forEach((item, index) => item.input
      .forEach((inputValue, inputIndex) => {
        if (inputValue.name === 'nominee_dob') {
          ...
          const tempDetails = userDetails?.update_request_meta?.bank_details;
          console.log('tempDetails', tempDetails);
          if (userDetails.enter_bank_details === 'Yes' || userDetails.enter_bank_details === 'No') {
          //  userDetails.enter_bank_details === 'No' -> edit case
          //  userDetails.enter_bank_details === 'Yes' -> initial as well as edit case
            ...
            if ([
              ...
              tempForm.form[index].input[inputIndex].defaultValue = tempDetails?.[inputValue.name] ?? userDetails[inputValue.name];
            }

            if (['passbook_cheque_url', 'proprietorship_proof'].includes(inputValue.name) && (tempDetails?.is_bank_verified === false || userDetails?.is_bank_verified === false)) {
              tempForm.form[index].input[inputIndex].condition.isShow = true;
              tempForm.form[index].input[inputIndex].defaultValue = tempDetails?.[inputValue.name] ? tempDetails[inputValue.name] : userDetails?.[inputValue.name];
            }

            if (inputValue.name === 'bankAccountVerificationStatus') {
              tempForm.form[index].input[inputIndex].defaultValue = (tempDetails?.is_bank_verified ?? userDetails?.is_bank_verified) ? 'VALID' : 'INVALID';
            }

            // making upi fields invisible
            if ([
              'upi_ifsc',
              'upi_bank_name',
              'upi_branch_name',
            ...
            if (['passbook_cheque_url', 'proprietorship_proof'].includes(inputValue.name)) {
              tempForm.form[index].input[inputIndex].condition.isShow = false;
              tempForm.form[index].input[inputIndex].defaultValue = tempDetails ? tempDetails[inputValue.name] : userDetails[inputValue.name];
            }
            ...
            if ([
              ...
              if (inputValue.name === 'confirmed_with_customer' && (userDetails?.is_bank_verified || tempDetails?.is_bank_verified)) {
                tempForm.form[index].input[inputIndex].defaultValue = 'Yes';
              } else {
                tempForm.form[index].input[inputIndex].defaultValue = tempDetails ? tempDetails[mapping[inputValue.name]] : userDetails[mapping[inputValue.name]];
              }
          ...
          if (['id_proof_url', 'address_proof_url'].includes(inputValue.name)) {
            if (userDetails[inputValue.name]?.length) {
              const pathPreview = userDetails[inputValue.name]?.[0].split('.');
